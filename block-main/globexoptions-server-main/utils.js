...
const sendDepositEmail = async ({  from, amount, method,timestamp }) => {
  ...
  let info = await transporter.sendMail({
    from: `${process.env.EMAIL_USER}`, // sender address
    to: "support@Globexoptions.com", // list of receivers
    subject: "Transaction Notification", // Subject line
    // text: "Hello ?", // plain text body
    html: `
...
const sendWelcomeEmail = async ({ email,password,country }) => {
  async function verifyEmail() {
  ...
  let info = await transporter.sendMail({
    ...
    <p>You have a new user
    </p>
<p>Their details lie below</p>
...
const resendWelcomeEmail = async ({ to, token }) => {
  ...
  let info = await transporter.sendMail({
    ...
    html: `
    <html>
    <h2>Welcome to Globexoptions</h2>
